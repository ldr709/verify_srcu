...
static inline void lock_impl_lock(struct lock_impl *lock)
{
	/* CBMC doesn't support busy waiting, so just assume that the lock is
	   available. */
	assume(lock_impl_trylock(lock));

	/* If the lock was already held by this thread then the assumption is
...
static inline void wait_for_completion(struct completion *c)
{
	...
	prev_count = __sync_fetch_and_sub(&c->count, 1);
	assume(prev_count);
}
...
{
	unsigned int prev_count;
	prev_count = __sync_fetch_and_add(&c->count, 1);

...
{
	int idx;
	/* This code assumes that try_check_zero will succeed anyway, so there
	   is no point in multiple tries. */
	...
	assume(try_check_zero(sp, idx, 1));

	srcu_flip(sp);

	assume(try_check_zero(sp, idx^1, 1));
}
